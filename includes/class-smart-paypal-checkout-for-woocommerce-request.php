...
class Paypal_Checkout_For_Woocommerce_Request extends WC_Payment_Gateway {
    ...
    public function __construct() {
        try {
            ...
            if (false === get_transient('psb_is_webhook_process_started')) {
                ...
                if (empty($webhook_id)) {
                    $this->psb_create_webhooks_request();
                }
            }
    ...
    public function psb_update_order($order) {
        try {
            ...
            $body_request = psb_remove_empty_key($purchase_units);
            $patch_request[] = array(
                'op' => 'replace',
            ...
            $this->psb_log('Endpoint: ' . $this->paypal_order_api . $paypal_order_id);
            $this->psb_log('Request: ' . print_r($patch_request, true));
    ...
    public function psb_create_webhooks_request() {
        try {
            ...
            if (is_wp_error($response)) {
                delete_transient('psb_is_webhook_process_started');
                $error_message = $response->get_error_message();
                $this->psb_log('Error Message : ' . wc_print_r($error_message, true));
            } else {
                ob_start();
